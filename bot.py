...
def back_kb():
    return InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ В МЕНЮ", callback_data="home")]])


def day_kb(n, status):
    label = "▶️ НАЧАТЬ" if status == "AVAILABLE" else ("▶️ ПРОДОЛЖИТЬ" if status == "IN_PROGRESS" else "↩️ ОТКРЫТЬ ДЕНЬ")
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(label, callback_data=f"startday:{n}")],
...
def reflection_feedback(day, text, uid=None):
    """Short supportive feedback that reacts to the user's actual reflection."""
    t = text.strip().lower()

    if day == 1:
        if any(p in t for p in ("скрыть", "скрывала", "скрывать", "спрятать", "не показать", "не хотела, чтобы", "чтобы не увид", "стыдно", "стыд")):
            return ("Спасибо, что написала это честно ❤️\n\n"
                    "Ты заметила важную вещь: тебе хотелось скрыть часть своего питания. "
                    "Это не повод себя ругать. Именно такие моменты нам сейчас важно увидеть. "
                    "Сегодня мы ничего не исправляем и не оцениваем — просто учимся замечать реальность такой, какая она есть. "
                    "Это и есть первый шаг к самостоятельности.")
        if any(p in t for p in ("перекус", "перекусы", "кусоч", "кофе", "соус")) and any(p in t for p in ("не замеч", "забыла", "не счит", "не учиты", "не дум", "оказыва", "на самом деле")):
            return ("Вот это как раз то наблюдение, ради которого нужен первый день ❤️\n\n"
                    "Ты увидела, что часть еды раньше просто оставалась вне поля внимания — например, перекусы или небольшие добавления. "
                    "Не нужно сейчас это исправлять или компенсировать. Наша задача — сначала научиться видеть свой рацион целиком. "
                    "Ты уже сделала первый шаг.")
        if any(p in t for p in ("мало ела", "ела мало", "почти не ела", "мало кушала", "не доедала")):
            return ("Спасибо, что заметила это ❤️\n\n"
                    "Ты увидела не только отдельные продукты, но и общую картину своего питания. "
                    "Сегодня ничего менять не нужно — нам важно собрать честное наблюдение без попытки сделать его «правильным». "
                    "Чем точнее ты видишь свой реальный день, тем легче будет дальше принимать решения самостоятельно.")
        if any(p in t for p in ("хотелось изменить", "изменить выбор", "чтобы увид", "тренер увид", "бот увид", "оцен", "правильн", "идеальн")):
            return ("Это очень важное наблюдение ❤️\n\n"
                    ...
                    "Сегодня ничего исправлять не нужно. Оставляем этот момент таким, какой он был. "
                    "Ты уже тренируешь самостоятельность: замечать реальность, даже когда хочется её отредактировать.")
        if any(p in t for p in ("удив", "не ожид", "впервые замет", "раньше не замеч")):
    ...
    if day == 2:
        answers = db.get_day_answers(uid, 2) if uid is not None else []
        hunger = [int(r["value"]) for r in answers if r["kind"] == "hunger_before"]
        satiety = [int(r["value"]) for r in answers if r["kind"] == "satiety_after"]
        parts = []

        if hunger and satiety:
            parts.append(f"Ты уже сделала {min(len(hunger), len(satiety))} отметк{'у' if min(len(hunger), len(satiety)) == 1 else 'и'} голода и сытости.")
            parts.append("Здесь не было задачи выбрать «правильные» цифры. Важно было остановиться и заметить ощущения до и после еды.")

        if any(p in t for p in ("утром", "утро", "с утра")) and any(p in t for p in ("голод", "голодна", "голодный", "есть хот")):
            parts.append("Ты заметила, что утром голод был сильнее. Это полезное наблюдение: теперь можно дальше смотреть, как ощущения меняются в течение дня.")

        if any(p in t for p in ("вечер", "вечером", "к вечеру")):
            if any(p in t for p in ("устал", "усталость", "слаб", "смес", "всё подряд", "все подряд")):
                parts.append("Ты также связала вечернее желание есть с усталостью. Не нужно сразу делать вывод, что причина только в ней — просто оставляем это как гипотезу и наблюдаем дальше.")
            else:
                parts.append("Ты заметила, что вечером ощущения меняются. Это именно тот материал, который помогает лучше понимать свои привычные сценарии.")

        if any(p in t for p in ("по времени", "время", "режим", "часам")):
            parts.append("Ты заметила влияние времени или привычного режима. Теперь важно не спорить с этим, а увидеть, как часто это повторяется.")

        if any(p in t for p in ("эмоци", "стресс", "нерв", "тревог", "скуч", "устал")):
            parts.append("Ты заметила эмоциональный контекст еды. Это не означает, что есть из-за эмоций «плохо» — нам важно научиться различать разные сигналы.")

        if any(p in t for p in ("сыт", "сыто", "наел", "переел", "не наел", "голоден")):
            parts.append("Отдельно важно, что ты начала замечать не только голод, но и момент насыщения. Именно это постепенно помогает принимать решения не только по времени или привычке.")

        if parts:
            return "Спасибо за честный ответ ❤️\n\n" + "\n\n".join(parts) + "\n\nСегодня тебе не нужно ничего исправлять. Ты тренируешь навык замечать сигналы тела и собирать собственные наблюдения."

        return ("Спасибо за честный ответ ❤️\n\n"
                "Сегодня мы не ищем идеальную цифру голода или сытости. Ты учишься останавливаться и замечать, что происходит с тобой до и после еды. "
                "Даже если пока сложно различать сигналы — это нормально. Сам факт наблюдения уже тренирует нужный навык.")

    return ("Спасибо за честный ответ ❤️\n\n"
            "Ты не обязана отвечать идеально. Здесь важно замечать себя, находить свои закономерности и постепенно учиться принимать решения самостоятельно.")
...
async def menu(update, context):
    ...
    if data == "home":
        await q.message.reply_text("Главное меню ❤️", reply_markup=main_kb())
    elif data == "continue":
        await show_day(q)
    elif data == "map":
        await show_map(q)
        ...
        await ask_system(q, data.split(":", 1)[1], context)
    elif data == "snacks":
        await show_snacks(q)
    elif data == "trainer":
        context.user_data["awaiting_question"] = True
        await q.message.reply_text("Напиши вопрос одним сообщением — я передам его тренеру.", reply_markup=back_kb())
    elif data.startswith("startday:"):
        await begin_day(q, int(data.split(":")[1]))
    elif data == "practice":
        await show_practice(q, context)
    elif data == "photo":
        ...
        await start_reflection(q, context)
        ...
        db.save_answer(uid, n, "hunger_before", value)
        ...
        value = int(data.split(":")[1])
        n = db.user(uid)["current_day"]
        db.save_answer(uid, n, "satiety_after", value)
        context.user_data.pop("awaiting_satiety", None)
        await q.message.reply_text(
            "🌿 Записала. Ты можешь отметить ещё один приём пищи, чтобы увидеть несколько эпизодов, или перейти к рефлексии.",
            reply_markup=hunger_next_kb()
        )
...
async def show_practice(q, context):
    uid = q.from_user.id
    n = db.user(uid)["current_day"]
    _, practice, _ = task_info(n)

    if n == 2:
        text = (
            "📝 <b>ПРАКТИКА</b>\n\n"
            "Перед несколькими приёмами пищи отметь свой голод, а после еды — сытость.\n\n"
            "Не оценивай себя. Просто фиксируй ощущения.\n\n"
            "Сделай хотя бы одну пару отметок. После каждой пары бот предложит либо записать ещё один приём пищи, либо перейти к рефлексии."
        )
        buttons = [
            [InlineKeyboardButton("🍽️ ОТМЕТИТЬ ГОЛОД", callback_data="hunger_start")],
            [InlineKeyboardButton("🤔 МНЕ СЛОЖНО", callback_data="trainer")]
        ]
    else:
...
async def finish_day(q, context, skipped=False):
    ...
    if db.complete_day(q.from_user.id, n, reflection):
        db.add_xp(q.from_user.id, info[4])
        badge = BADGES.get(n)
        badge_text = ""
        if badge and db.add_badge(q.from_user.id, badge):
        ...
        await q.message.reply_text("Этот день уже завершен ❤️", reply_markup=main_kb())
...
async def handle_text(update, context):
    ...
    if field:
        db.set_system_item(uid, field, text)
        await update.message.reply_text("Сохранила ❤️\n\nТвою систему можно дополнять в любой момент.", reply_markup=main_kb())
        return
    ...
    if context.user_data.pop("awaiting_reflection", False):
        ...
        if db.complete_day(uid, n, text):
            feedback = reflection_feedback(n, text, uid)
            db.add_xp(uid, info[4])
            badge = BADGES.get(n)
            badge_text = ""
            if badge and db.add_badge(uid, badge):
                badge_text = f"\n🏆 Новое достижение: {badge}"
...
async def handle_photo(update, context):
    ...
    if not context.user_data.pop("awaiting_photo", False):
        await update.message.reply_text("Фото получено ❤️ Если это фотоотчет по текущему дню, открой практику.")
        return
    u = db.user(uid)
    n = u["current_day"]
    ...
    for admin in ADMIN_IDS:
        try:
            ...
            pass
    await update.message.reply_text(
        "Фотоотчет сохранен ❤️\n\n"
        "Я получила его. Сегодня ничего не нужно исправлять — нам важно увидеть реальный день таким, какой он был.\n\n"
        "Когда будешь готова, перейди к рефлексии: я помогу тебе разобрать наблюдение и дам короткую обратную связь.",
        reply_markup=after_photo_kb()
    )
...
async def show_skills(q):
    uid = q.from_user.id
    done = sum(1 for n in range(1, 50) if db.day_row(uid, n)["status"] == "COMPLETED")
    text = "🧠 <b>МОИ НАВЫКИ</b>\n\n"
...
async def show_badges(q):
    ...
    await q.message.reply_text(
        "🏆 <b>ДОСТИЖЕНИЯ</b>\n\n" + ("\n".join("• " + x for x in b) if b else "Пока нет — они будут появляться по ходу курса."),
        parse_mode="HTML",
...
async def show_progress(q):
    ...
    percent = round(done / 49 * 100)
    await q.message.reply_text(
        f"📊 <b>МОЙ ПРОГРЕСС</b>\n\n🗓 {done}/49 дней\n📈 {percent}%\n⭐ {u['xp']} XP\n🎯 Текущий день: {u['current_day']}\n🏆 Достижений: {len(db.badges(uid))}",
        parse_mode="HTML",
        reply_markup=back_kb()
    )
...
async def show_system(q):
    items = db.system_items(q.from_user.id)
    text = "❤️ <b>МОЯ СИСТЕМА</b>\n\nЭто твоя личная инструкция. Она собирается по ходу курса.\n\n"
    for key, label in SYSTEM_FIELDS:
        text += f"{label}: {items.get(key, '—')}\n"
    buttons = [[InlineKeyboardButton(label, callback_data=f"sys:{key}")] for key, label in SYSTEM_FIELDS]
    buttons.append([InlineKeyboardButton("⬅️ В МЕНЮ", callback_data="home")])
    await q.message.reply_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(buttons))


async def ask_system(q, field, context):
    label = next((label for key, label in SYSTEM_FIELDS if key == field), field)
    context.user_data["awaiting_system"] = field
...
async def reminders(context):
    ...
    if now.hour != REMINDER_HOUR:
        return
    con = db.connect()
    rows = con.execute("SELECT * FROM users WHERE current_day<=49").fetchall()
    con.close()
    for u in rows:
        if not db.claim_reminder(u["tg_id"], date_key):
            continue
        try:
            await context.bot.send_message(
                u["tg_id"],
                f"🌿 Добрый день, {u['name']}!\nТвой День {u['current_day']} из 49 ждёт тебя.",
                reply_markup=main_kb()
            )
        except Exception:
            pass
...
def main():
    ...
    if app.job_queue:
        app.job_queue.run_repeating(reminders, interval=60, first=5)
    app.run_polling()
