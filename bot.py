...
def reflection_feedback(day, text):
    """Supportive, short feedback that reacts to concrete observations in the reflection."""
    t = text.strip().lower()
    if day == 1:
        if any(phrase in t for phrase in ("скрыть", "скрывала", "скрывать", "спрятать", "не показать", "не хотела, чтобы", "чтобы не увид", "стыдно", "стыд")):
            return ("Спасибо, что написала это честно ❤️\n\n"
                    "Ты заметила важную вещь: тебе хотелось скрыть часть своего питания. "
                    "Это не повод себя ругать. Наоборот — именно такие моменты нам сейчас важно увидеть. "
        ...
        if any(phrase in t for phrase in ("перекус", "перекусы", "кусоч", "кофе", "соус")) and any(phrase in t for phrase in ("не замеч", "забыла", "не счит", "не учиты", "не дум", "оказыва", "на самом деле")):
        ...
        if any(phrase in t for phrase in ("мало ела", "ела мало", "почти не ела", "мало кушала", "не доедала")):
        ...
        if any(phrase in t for phrase in ("хотелось изменить", "изменить выбор", "чтобы увид", "тренер увид", "бот увид", "оцен", "правильн", "идеальн")):
